...
class vLLMAnswerEvaluationRollout(vLLMAutoraterRollout):
    """
    vLLM Answer Evaluation Rollout that implements answer generation with autorater feedback.
    ...
    def evaluate_answer_correctness(self, explicit_task: str, answer: str, prompt_idx: int = 0, meta_info: Dict = None) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            autorater_decisions, autorater_explanations, autorater_raw_responses = call_autorater_service(
                self.answer_evaluation_service_url, autorater_payload, batch_size=1
            )
...
Please provide a new, improved answer that correctly addresses the question."""
        ...
        messages = [
            format_system_message("answer_first"),
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        while active_indices:
            ...
            for i, prompt_idx in enumerate(batch_prompt_indices):
                ...
                print(f"    Prompt {prompt_idx}: Extracted answer ({len(extracted_answer)} chars)")
                ...
                if meta_info and "explicit_tasks" in meta_info:
                    ...
                    if prompt_idx < len(explicit_tasks) and explicit_tasks[prompt_idx]:
                        ...
                        print(f"      Using explicit task: {explicit_task[:100]}...")
                    else:
                        print(f"      Using original question as explicit task: {explicit_task[:100]}...")
                else:
                    print(f"      Using original question as explicit task: {explicit_task[:100]}...")
                
                # Evaluate answer correctness
                is_correct, confidence, explanation = self.evaluate_answer_correctness(
                    explicit_task, extracted_answer, prompt_idx, meta_info
                )
                ...
                if is_correct:
                    ...
                    print(f"      ✅ Answer approved! Stopping generation for prompt {prompt_idx}")
                    ...
                    if generation_history[prompt_idx]['total_regeneration_attempts'] < self.max_regeneration_attempts:
                        ...
                        remaining_tokens = self.config.response_length - current_length
                        
                        if remaining_tokens > 50:  # Need at least 50 tokens for meaningful regeneration
                            print(f"      ❌ Answer rejected. Attempts: {generation_history[prompt_idx]['total_regeneration_attempts']}/{self.max_regeneration_attempts}")
                            print(f"      🔄 Remaining tokens: {remaining_tokens}. Will try again...")
                            
                            # Create reprompt with rejected answer
                            reprompt = self.create_reprompt_with_rejected_answer(
                                original_prompts[prompt_idx],
                                extracted_answer,
                                generation_history[prompt_idx]['total_regeneration_attempts']
                            )
                            
                            # Update current input with reprompt
                            ...
                            curr_max_tokens[prompt_idx] = remaining_tokens
                            
                            # Keep this prompt active for next iteration
                            new_active_indices.append(prompt_idx)
                            
                        else:
                            print(f"      ⚠️  Answer rejected but insufficient tokens remaining ({remaining_tokens}). Stopping generation.")
                            generation_history[prompt_idx]['final_response'] = generated_response
                        ...
                        print(f"      ⚠️  Max regeneration attempts ({self.max_regeneration_attempts}) reached. Stopping generation.")
                        generation_history[prompt_idx]['final_response'] = generated_response
                        generation_history[prompt_idx]['final_answer_approved'] = False
            ...
            for prompt_idx in active_indices:
                if len(curr_inputs[prompt_idx]) - len(init_inputs[prompt_idx]) >= self.config.response_length:
                    ...
                    print(f"    Prompt {prompt_idx}: Reached max length, truncating")
                else:
        ...
        for i in range(batch_size):
            ...
            print(f"  Total attempts: {generation_history[i]['total_regeneration_attempts']}")
            print(f"  Answer approved: {generation_history[i]['final_answer_approved']}")
