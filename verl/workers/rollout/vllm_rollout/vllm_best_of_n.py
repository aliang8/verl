...
class vLLMBestOfN(vLLMRollout):
    ...
    def __init__(self, model_path: str, config, tokenizer, model_hf_config, **kwargs):
        ...
        self.n_candidates = config.get("n_candidates", 10)  # Number of candidates to generate
        ...
        self.use_random_selection = config.get("use_random_selection", False)  # Use random selection instead of oracle
        self.random_seed = config.get("random_seed", 42)  # Seed for reproducible random selection
        ...
        print(f"Initialized vLLMBestOfN with n_candidates={self.n_candidates}")
        print(f"Plan selection: {'Random' if self.use_random_selection else 'Oracle/Autorater'}")
    ...
    def evaluate_response(self, question: str, candidates: List[str], prompt_idx: int = 0, meta_info: Dict = None) -> Tuple[int, float]:
        """Use the autorater service to select the best candidate plan, or random selection if configured."""
        # Handle random selection if configured
        if self.use_random_selection:
            # Set seed for reproducible random selection
            random.seed(self.random_seed + prompt_idx)  # Add prompt_idx for different seeds per prompt
            
            # Select a random candidate
            best_idx = random.randint(0, len(candidates) - 1)
            best_score = 0.5  # Default score for random selection
    ...
    def process_candidates_for_turn(self, candidates: List[str], prompt_idx: int, question: str, init_inputs: List, curr_inputs: List, turn: int) -> Tuple[List[Dict], List[Dict]]:
        """
        ...
        for candidate_idx, current_turn_generation in enumerate(candidates):
            ...
            if self.answer_stop_token in full_generation_text:
                ...
                candidates_without_answers.append({"candidate_idx": candidate_idx, "current_turn_generation": current_turn_generation})
                print(f"  Candidate {candidate_idx + 1}: No </answer> reached yet")

        return candidates_with_answers, candidates_without_answers

    def generate_text_with_model(self, prompt: str, num_outputs: int = 1, max_new_tokens: int = 256, temperature: float = 0.8, top_p: float = 0.9, seed: int = None) -> List[str]:
        """
        ...
        try:
            ...
            with self.update_sampling_params(max_tokens=max_new_tokens, temperature=temperature, top_p=top_p, n=num_outputs, stop=[self.answer_stop_token] if self.answer_stop_token else None, detokenize=True if self.answer_stop_token else None, seed=seed):
                outputs = self.inference_engine.generate(prompts=[prompt_input], sampling_params=self.sampling_params, use_tqdm=False)
    ...
    def generate_text_with_model_batch(self, prompts: List[str], num_outputs: int = 1, max_new_tokens_list: List[int] = None, temperature: float = 0.8, top_p: float = 0.9, seeds: List[int] = None) -> List[List[str]]:
        """
        ...
        try:
            ...
            with self.update_sampling_params(
                ...
                outputs = self.inference_engine.generate(prompts=prompt_inputs, sampling_params=self.sampling_params, use_tqdm=False)
    ...
    def generate_candidates_for_prompt(self, prompt_idx: int, curr_inputs: List, curr_max_tokens: List) -> List[str]:
        ...
        candidates = self.generate_text_with_model(prompt=prompt_text, num_outputs=self.n_candidates, max_new_tokens=curr_max_tokens[prompt_idx], temperature=1.2, top_p=0.9, seed=prompt_idx)
    ...
    def generate_diverse_answers_batch(self, prompt: str, existing_answers: List[str], num_to_generate: int, max_new_tokens: int = 256) -> List[str]:
        ...
        if not existing_answers:
            ...
            return self.generate_text_with_model(prompt=prompt, num_outputs=num_to_generate, max_new_tokens=max_new_tokens, temperature=0.8, top_p=0.9)
        ...
        diverse_prompt = f"""{prompt}Generate a different answer that is DIFFERENT from the existing approaches shown below.
Existing approaches to avoid:
{chr(10).join(negative_examples)}<im_end>\n<im_start|>assistant\n"""

        # Generate diverse answers using the generic function
        new_responses = self.generate_text_with_model(prompt=diverse_prompt, num_outputs=10, max_new_tokens=1024, temperature=1.2, top_p=0.9)
    ...
    def create_synthetic_candidate(self, answer: str, candidate_idx: int, candidates: List, prompt_idx: int, curr_inputs: List, init_inputs: List, full_response: str = None) -> Dict:
        """
    ...
    def continue_from_candidate(self, candidate: str, prompt_idx: int, curr_inputs: List, init_inputs: List, curr_max_tokens: List, new_active_indices: List) -> None:
        """
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Force answer completion if needed for prompts that hit max length without generating a plan
        if hasattr(self, 'force_answer_completion') and self.force_answer_completion:
            ...
            for i, response_ids in enumerate(response_list):
                ...
                # Check if we have at least 1 complete answer block (for best_of_n, we expect at least 1)
                if answer_blocks < 1 or answer_end_blocks < 1:
                    print(f"  Prompt {i}: Incomplete answer (found {answer_blocks} <answer> and {answer_end_blocks} </answer>), forcing completion")
                    
                    # Force answer completion by adding </think><answer> if not present
                    if not response_text.endswith("</answer>"):
                        if not response_text.endswith("</think>"):
                            response_text += "</think>"
                        if not response_text.endswith("<answer>"):
                            response_text += "<answer>"
                        # Add </answer> to force the model to complete the answer
                        response_text += "</answer>"
                ...
                # else:
                #     print(f"  Prompt {i}: Complete answer (found {answer_blocks} <answer> and {answer_end_blocks} </answer>)")
            ...
            if to_force_answer_indices:
                ...
                additional_tokens = getattr(self, 'additional_answer_tokens', 512)
