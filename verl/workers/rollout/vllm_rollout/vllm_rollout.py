...
New plan:"""
    ...
    def _generate_single_plan(self, prompt: str, max_new_tokens: int = 256) -> str:
        ...
        try:
            ...
            with self.update_sampling_params(max_tokens=max_new_tokens, temperature=0.8, top_p=0.9, n=1, stop=[self.answer_stop_token] if self.answer_stop_token else None, detokenize=True if self.answer_stop_token else None, seed=hash(prompt) % 10000):
                outputs = self.inference_engine.generate(prompts=[prompt_input], sampling_params=self.sampling_params, use_tqdm=False)
