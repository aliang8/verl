...
class ValidationRolloutGenerator:
    ...
    def __init__(self, checkpoint_path: str, config: Optional[DictConfig] = None, rollout_config: Optional[Dict] = None):
        """
    ...
    def _is_huggingface_model_id(self, model_path: str) -> bool:
        ...
        return "/" in model_path and not Path(model_path).exists() and not model_path.startswith("./") and not model_path.startswith("../") and not model_path.startswith("/")
        ...
        for raw_prompt in raw_prompts:
            ...
            try:
                if hasattr(self.tokenizer, "apply_chat_template"):
                    formatted_prompt = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, enable_thinking=True)
                else:
    ...
    def generate_rollouts(self, val_data_paths: List[str], output_dir: Optional[str] = None, step: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        if num_trajectories > 1:
            sampling_params = SamplingParams(temperature=temperature, top_p=top_p, max_tokens=max_tokens, stop=self.rollout_config.get("stop_tokens", GENERATION_CONFIG["stop_tokens"]), n=num_trajectories, seed=None if temperature > 0 else 42)
            ...
            for prompt_idx, (raw_prompt, formatted_prompt, output) in enumerate(zip(raw_prompts, val_prompts, batch_outputs)):
                for traj_idx, completion in enumerate(output.outputs):
                    single_output = type(output)(request_id=f"{output.request_id}_{traj_idx}", prompt=output.prompt, prompt_token_ids=output.prompt_token_ids, prompt_logprobs=output.prompt_logprobs, outputs=[completion], finished=output.finished)
            ...
            sampling_params = SamplingParams(temperature=temperature, top_p=top_p, max_tokens=max_tokens, stop=self.rollout_config.get("stop_tokens", GENERATION_CONFIG["stop_tokens"]), n=1, seed=None if temperature > 0 else 42)
...
def main():
    ...
    parser.add_argument("--config", type=str, default=None, help="Path to training config file (optional)")
    parser.add_argument("--val_data", type=str, default=None, help="Validation data file(s). Single: 'file.parquet' or multiple: \"['file1.parquet','file2.parquet']\"")
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory (defaults to checkpoint_dir/val_rollouts)")
    parser.add_argument("--step", type=int, default=None, help="Step number for output organization")
    ...
    generator = ValidationRolloutGenerator(checkpoint_path=args.checkpoint_dir, config=config, rollout_config=rollout_config)

    # Generate rollouts
    try:
        results = generator.generate_rollouts(val_data_paths=val_data_paths, output_dir=args.output_dir, step=args.step)
